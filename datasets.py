...
class BonnFurniture(BaseDataset):
  """Bonn Furniture dataset."""

  def __init__(self, path, split='train_split', transform=None):
    super(BonnFurniture, self).__init__()
    ...
    #Get all metadata. Should we be grabbing from splits instead?
  ...
  for filename in label_files:
      ...
      with open(label_path + '/' + filename) as f:
        lines = f.readlines()
      for line in lines:
        #If we split the line this way as Fashion200k did, Metadata is grabbed by index [5]
        linehalfOne = line.split('    ')
        img = {
            'file_path': linehalfOne[1],
            'style': linehalfOne[0],
            'Metadata': [caption_post_process(linehalfOne[5])],
            'split': split,
            'modifiable': False
        }
        self.imgs += [img]
    print 'BonnFurniture:', len(self.imgs), 'images'
